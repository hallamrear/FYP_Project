...
void Particle::ResolveCollision(Particle* particle)
{
	...
	float thetaOne = atan(GetModel()->GetVelocity().y / GetModel()->GetVelocity().x) * (180 / M_PI);
	float thetaTwo = atan(particle->GetModel()->GetVelocity().y / particle->GetModel()->GetVelocity().x) * (180 / M_PI);
